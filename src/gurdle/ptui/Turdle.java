...
public class Turdle extends ConsoleApplication
                ...
                throw new Exception(
                        String.format(
                                "\"%s\" is not the required word length (%d)." +
                                        System.lineSeparator(), firstWord,
                                Model.WORD_SIZE));
            }
        } else {
            this.model.newGame();
    ...
    public void start(PrintWriter out) {
        ...
        super.setOnCommand("guess", 1, "<word>: Make " +
                "a guess", args -> this.model.enterNewGuess(args[0]));
        super.setOnCommand("cheat", 0, ": Show the " +
                "secret word", args -> this.cheat());
        super.setOnCommand("new", 0, "Start a new " +
                "game", args -> this.model.newGame());
    }
